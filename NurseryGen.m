% Function to generate the Nursery
function ngen = NurseryGen(R,C,Xmax,Ymax)

rng('shuffle'); %init random generator
global bitmap RS nursery;

K = 3; % number of tree rows running south-north direction
M = 7; %maximum number of trees in each row
W = RS; % distance between tree rows (m)
...
figure(1); 
bitmap = imnoise(bitmap,'salt & pepper',0.02); % Adding random white noise 
% to simulate tall grass or twigs near the ground 
...
set(gca,'YDir','normal');
axis equal
title('Generated Tree Nursery in the World Frame')
